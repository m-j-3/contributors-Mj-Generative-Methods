...
class DarkSystem extends ParticleSystem {
  static label = "⬛"; // Ignore the Glitch parse error
  static desc = "You Are Not Welcome Here.";
  constructor() {
    // Make wind particles
    super(DarkParticle, 100);
    this.windScale = 0.01;
  }
...
class DarkParticle extends Particle {
  ...
  calculateForces(p, dt) {
   ...
   this.v.mult(0.99);
    ...
    let windDir = 20 * p.noise(t * 0.1, this.idNumber);
    ...
    // Spin with the wind!
    this.angle += Math.sin(windDir) * 0.1;
  ...
  draw(p, drawDebug = false) {
    ...
    p.scale(1, 0.5 + 0.5 * Math.cos(this.idNumber + t * 2) + 0.1);

    let leafWidth = 100;
    let leafLength = 20;
    p.beginShape();
    p.vertex(-leafLength * 0.5, 0);
    p.curveVertex(leafLength * -0.1, -leafWidth);
    p.vertex(leafLength * 0.5, 0);
    p.curveVertex(leafLength * -0.1, leafWidth);
    p.vertex(-leafLength * 0.5, 0);
    p.endShape(p.CLOSE);
