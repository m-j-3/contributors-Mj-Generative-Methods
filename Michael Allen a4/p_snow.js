...
class SnowSystem extends ParticleSystem {
  ...
  static desc = "A bunch of snow balls that fly around the screen and the more wild you move your mouse the more intense the snow ball fight gets"; // Ignore the Glitch parse error

  constructor() {
    // Make what particle, and how many?
    // Try different numbers of particles
    super(SnowParticle, 200);
  }

  draw(p) {
    // A little bit of trails!
    p.background(0, 0, 50, 50);
...
class SnowParticle extends Particle {
  ...
  calculateForces(p, dt) {
   ...
   // this.attractionForce2 = this.pos.getForceTowardsPoint(center, random(0.1), {
     // falloff: 1,
    //});
    ...
    this.attractionForce = this.pos.getForceTowardsPoint(mouse, 1.5, {
      falloff: 1.2,
    });
